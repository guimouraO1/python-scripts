...
# lista e move os arquivos do dia passado PARA MOVER APENAS ARQUIVOS COM UM DIA PASSADO
def lista_e_move(origem, destino, ano, dia):

    # lista os arquivos na origem.
    arquivos = obtem_lista_arquivos(origem)
    # Verficando arquivo por arquivo que contém o dia e ano correspondente 
    for arquivo in arquivos:
        # se o arquivo tiver o "e" o ano e o dia correspondente ao passsado ele coloca na pasta de destino
        if f'e{ano}{dia}' in arquivo:
            verifica_destino(destino)
            # obtem a origem com o arquivo
            origem_arquivo = os.path.join(origem, arquivo)
            # obtem o destino com o arquivo
            destino_arquivo = os.path.join(destino, arquivo)
            # move o arquivo
            shutil.move(origem_arquivo, destino_arquivo)
            # log de verificação
            logging.info(f'Arquivo {arquivo} movido para ano{ano}/dia{dia}')
        else:
            logging.info(f'Arquivo {arquivo} não corresponde a pesquisa')
...
def movendo_arquivos(origem, quantidade_arquivos, lista_arquivos):
    ...
    for x in range(0, quantidade_arquivos):
        ...
        if match:
            ...
            destino = Path(f'{origem}/dia{dia_txt}')
